...
public abstract class AbstractKeycardItem extends Item {
    ...
    public AccessNetwork getAccessNetwork(ItemStack stack, @Nullable Level level) {
        CompoundTag tag = AxessItemData.get(stack);
    ...
    public AccessLevel getAccessLevel(ItemStack stack, @Nullable Level level) {
        ...
        return net.getAccessLevel(tag.getUUID(ACCESS_LEVEL_KEY));
    }

    public void setAccessNetwork(ItemStack stack, AccessNetwork network) {
        AxessItemData.update(stack, tag -> tag.putUUID(ACCESS_NETWORK_KEY, network.getUUID()));
    }

    public void setAccessLevel(ItemStack stack, AccessLevel level) {
        AxessItemData.update(stack, tag -> tag.putUUID(ACCESS_LEVEL_KEY, level.getUUID()));
    }
    ...
    public void appendHoverText(ItemStack pStack, Item.TooltipContext pContext, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        ...
        AccessNetwork net = getAccessNetwork(pStack, Minecraft.getInstance().level);
        AccessLevel level = getAccessLevel(pStack, Minecraft.getInstance().level);
